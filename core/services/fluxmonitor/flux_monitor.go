...
func (fm *concreteFluxMonitor) actionConsumer() {
...
Loop:
	for {
		...
		case maybeLog := <-p.chMaybeLogs:
			if maybeLog.Err != nil {
				logger.Error(errors.WithStack(maybeLog.Err))
...
func (p *PollingDeviationChecker) respondToNewRoundLog(log *contracts.LogNewRound) error {
	...
	nextAnswer, err := p.fetcher.Fetch()
	if err != nil {
		return errors.Wrap(err, "unable to fetch median price")
	}

	return p.createJobRun(nextAnswer, roundState.ReportableRoundID)
}
...
func (p *PollingDeviationChecker) pollIfEligible(threshold float64) {
	...
	if !roundState.EligibleToSubmit {
		logger.Info("not eligible to submit, polling paused")
		return
	}
	...
	jobSpecID := p.initr.JobSpecID.String()
	promSetDecimal(promFMSeenValue.WithLabelValues(jobSpecID), nextAnswer)

	if !OutsideDeviation(utils.DecimalFromBigInt(roundState.LatestAnswer, p.precision), nextAnswer, threshold) {
		return
	}

	logger.Infow("Detected change outside threshold, starting new round",
		"reporatableRound", roundState.ReportableRoundID,
		"address", p.initr.Address.Hex(),
		"jobID", p.initr.JobSpecID,
	)
	err = p.createJobRun(nextAnswer, roundState.ReportableRoundID)
	if err != nil {
		logger.Errorf("can't create job run: %v", err)
		return
	}

	promSetDecimal(promFMReportedValue.WithLabelValues(jobSpecID), nextAnswer)
	promSetBigInt(promFMReportedRound.WithLabelValues(jobSpecID), roundState.ReportableRoundID)
}

func (p *PollingDeviationChecker) createJobRun(nextAnswer decimal.Decimal, nextRound *big.Int) error {
